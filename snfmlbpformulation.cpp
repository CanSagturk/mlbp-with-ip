...
void SNFMLBPFormulation::createDecisionVariables(IloEnv env, const Instance<MLBP>& inst)
{
...
void SNFMLBPFormulation::addConstraints(IloEnv env, IloModel model, const Instance<MLBP>& inst)
{
	/////////////// NETWORK FLOW TRIAL
	CPXENVptr envNF = NULL;
	CPXNETptr net = NULL;
	int       status;
	int       i, j;

	/* Initialize the CPLEX environment */
	envNF = CPXopenCPLEX(&status);

	/* Create the problem. */
	net = CPXNETcreateprob(envNF, &status, "netex1");
...
void SNFMLBPFormulation::addObjectiveFunction(IloEnv env, IloModel model, const Instance<MLBP>& inst)
{
	IloExpr sum(env);
	for (int i : inst.M) {
		for (int j : inst.B[i]) {
...
void SNFMLBPFormulation::extractSolution(IloCplex cplex, const Instance<MLBP>& inst, Solution<MLBP>& sol)
{
	sol.cost = 0;
	for (int i = 0; i <= inst.m; i++) {
		if (i == inst.m) {
