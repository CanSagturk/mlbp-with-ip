...
// Matthias Horn
#if defined(USER_MH) && (USER_MH > 0)
CREATE_USER(MH, logging::Blue, USER_MH);
#define MH_OUT(verbosity) if (true) user::dec_logger<user::MH>(verbosity)
#else
CREATE_USER(MH, logging::NoCol, 0);
#define MH_OUT(verbosity) if (false) user::null_logger<user::MH>()
#endif
...
// Can Sagturk
#if defined(USER_CS) && (USER_CS > 0)
CREATE_USER(CAN, logging::Magenta, USER_CS);
#define CS_OUT(verbosity) if (true) user::dec_logger<user::CAN>(verbosity)
#else
CREATE_USER(CAN, logging::Red, 0);
#define CS_OUT(verbosity) if (false) user::null_logger<user::CAN>()
#endif
